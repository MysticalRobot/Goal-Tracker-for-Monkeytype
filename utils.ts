import type { BrowserSyncStorage, Theme, NotificationFrequency, DailyGoalsMinutes } from './types';

export function getIntervalManager(callback: () => void, timeout: number) {
...
export async function getDailyGoalsMinutes(): Promise<DailyGoalsMinutes> {
  const dailyGoalsMinutesKey: keyof BrowserSyncStorage = 'dailyGoalsMinutes';
  const storage = await browser.storage.sync.get(dailyGoalsMinutesKey);
  const uninitializedDailyGoalsMinutes = !Object.hasOwn(storage, dailyGoalsMinutesKey);
  let dailyGoalsMinutes: DailyGoalsMinutes & BrowserSyncStorage[keyof BrowserSyncStorage];
  if (uninitializedDailyGoalsMinutes) {
    dailyGoalsMinutes = {...initialSyncStorage.dailyGoalsMinutes};
    await browser.storage.sync.set({ dailyGoalsMinutes });
  } else {
    dailyGoalsMinutes = storage.dailyGoalsMinutes;
  }
  return dailyGoalsMinutes;
}

export async function getNotificationFrequency(): Promise<NotificationFrequency> {
  const notificationFrequencyKey: keyof BrowserSyncStorage = 'notificationFrequency';
  const storage = await browser.storage.sync.get(notificationFrequencyKey);
  const uninitializedNotificationFrequency = !Object.hasOwn(storage, notificationFrequencyKey);
  let notificationFrequency: NotificationFrequency & BrowserSyncStorage[keyof BrowserSyncStorage];
  if (uninitializedNotificationFrequency) {
    notificationFrequency = initialSyncStorage.notificationFrequency;
    await browser.storage.sync.set({ notificationFrequency });
  } else {
    notificationFrequency = storage.notificationFrequency;
  }
  return notificationFrequency;
}

// modifies the icon svg with the theme's colors and creates a data URI to use it
export function getIconURI(theme: Theme): string {
  const icon = `<svg width="48" height="48" xmlns="http://www.w3.org/2000/svg">
...
// export const initialSyncStorage: BrowserSyncStorage = {
...
export const initialSyncStorage: BrowserSyncStorage = {
  ...
  notificationFrequency: 'quarterGoalCompletion'
};
